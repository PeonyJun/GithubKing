...
- Instructions:
  ...
  - vue3beta 重做后的架构：仓库列表在 src/views/ReposView.vue（mode=own/starred/other，13 个仓库长按菜单 action，支持 @用户/链接/公共仓库搜索并复用该列表）；文件浏览器抽成 src/components/FileBrowser.vue（路径/分支无关，18 个文件/文件夹长按菜单 action、多选、网格/列表视图），RepoDetailView(仓库根)和 FilesView(子目录+面包屑)都复用 FileBrowser；设置页含双代理前缀(访问 gh-proxy.org / 下载 down.ksx.qzz.io)、网格视图、排序、按 repo/folder/file 三 tab 开关长按菜单项。长按菜单定义与可见性过滤在 src/composables/useContextMenu.ts（复刻原 index.html 的 CONTEXT_MENU_DEFINITIONS）。
