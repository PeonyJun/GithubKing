...
- Instructions:
  ...
  - vue3beta 重做后的架构：仓库列表在 src/views/ReposView.vue（mode=own/starred/other，13 个仓库长按菜单 action，本地搜索仅名称过滤 + @用户/GitHub 链接跳转）；文件浏览器抽成 src/components/FileBrowser.vue（路径/分支无关，18 个文件/文件夹长按菜单 action、多选、网格/列表视图），RepoDetailView(仓库根)和 FilesView(子目录+面包屑)都复用 FileBrowser；设置页含双代理前缀(访问 gh-proxy.org / 下载 down.ksx.qzz.io)、网格视图、排序、按 repo/folder/file 三 tab 开关长按菜单项。长按菜单定义与可见性过滤在 src/composables/useContextMenu.ts（复刻原 index.html 的 CONTEXT_MENU_DEFINITIONS）。
  - 底部 Tabbar 为「仓库/搜索/部署/我的」4个 tab（TabLayout.vue）：搜索走独立 src/views/SearchView.vue（van-tabs 切类别：仓库/代码/用户/Issues，共用"筛选"底部 van-popup 面板、排序用 van-dropdown-menu，结果走 van-list 分页；代码搜索文件级展示 text_matches 片段）；部署走 src/views/DeployView.vue（我的仓库列表 + Pages 启用/切换分支/自定义域名CNAME）；动态/收藏不再是 tab——动态从抽屉账户框右上角铃铛图标进 /activity，收藏从抽屉账户名正下方星标快捷入口进 /starred。各仓库/文件列表顶部搜索框已简化为本地名称过滤，全局/代码/用户搜索统一收敛在"搜索"tab。搜索 qualifier 编译工具在 src/utils/searchQuery.ts。
